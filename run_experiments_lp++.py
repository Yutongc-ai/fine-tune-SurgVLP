...
# 遍历每个组合并运行实验
for idx, (attention_pooling, epochs, num_shot, init_alpha) in enumerate(all_combinations, 1):
    print(f"\n{'='*50}")
    print(f"Running experiment {idx}/{len(all_combinations)}")
    print(f"Config: attention_pooling={attention_pooling}, num_shots={num_shot}, epochs={epochs}, init_alpha={init_alpha}")
    print(f"{'='*50}")
