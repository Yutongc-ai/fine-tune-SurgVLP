...
class NegationMulDist(nn.Module):
    def __init__(self, configs, model, preprocess, tokenizer):
        ...
        self.lambda_loss = torch.tensor(1.0)
    ...
    def target_local_features(self, image, num_height=3, num_weight=3):
        # image tensor: [bs, 3, 224, 224] (例如 [7, 3, 224, 224])
        bs, c, h, w = image.shape

        # --- 代码修改开始 ---

        # 1. 定义补丁参数
        # 你指定的补丁大小 (kernel size)
        kernel_size = 96
        
        # 2. 计算 stride 以便从 224 中获得 3 个补丁
        # 3 = floor((224 - 96) / S) + 1  =>  2 = floor(128 / S)
        # S=64 是一个有效值 (128 / 64 = 2)
        stride = 64
        ...
        # 断言，确保我们的计算是正确的
        assert out_h == num_height, f"高度计算错误: 得到 {out_h} 而不是 {num_height} (K=96, S=64)"
        assert out_w == num_weight, f"宽度计算错误: 得到 {out_w} 而不是 {num_weight} (K=96, S=64)"
        
        output_patch_count = out_h * out_w # 3 * 3 = 9

        # 3. 使用 unfold 提取重叠补丁
        # F.unfold 输入 [bs, C, H, W]
        # F.unfold 输出 [bs, C * K * K, L]
        # L = 补丁数量 (即 9)
        # C * K * K = 3 * 96 * 96 = 27648
        # image_patch_unfolded 形状: [bs, 27648, 9]
        image_patch_unfolded = F.unfold(image, kernel_size=kernel_size, stride=stride)

        # 4. 重新塑形以匹配模型输入
        # [bs, C * K * K, L] -> [bs, C, K, K, L]
        image_patch_view = image_patch_unfolded.view(bs, c, kernel_size, kernel_size, output_patch_count)
        
        # [bs, C, K, K, L] -> [bs, L, C, K, K]
        image_patch_permuted = image_patch_view.permute(0, 4, 1, 2, 3)

        # [bs, L, C, K, K] -> [bs * L, C, K, K]
        # 最终形状: [bs * 9, 3, 96, 96]
        # 这可以被送入你的模型
        image_patch = image_patch_permuted.reshape(bs * output_patch_count, c, kernel_size, kernel_size)
        
        # --- 代码修改结束 ---

        with torch.no_grad():
            # 假设 self.model.extract_feat_img 期望 [N, 3, 96, 96] 的输入
            target_features, _ = self.model.extract_feat_img(image_patch)
        
        target_features = target_features.detach()
        
        # target_features 现在的形状是 [bs * 9, feature_dim]
        # 我们把它变回 [bs, 9, feature_dim]
        target_features = target_features.view(bs, output_patch_count, -1)        
    ...
    def self_dist(self, target_features, local_features):
        ...
        bs, _, embed_dim = local_features.shape
        local_features_grid = local_features.view(bs, 7, 7, embed_dim)
        # 3. 转换为 'channels-first' 格式
        # nn.AvgPool2d 需要 [bs, C, H, W] 格式
        # [bs, 7, 7, 768] -> [bs, 768, 7, 7]
        local_features_cf = local_features_grid.permute(0, 3, 1, 2)

        # 4. 定义并应用 2D 平均池化
        # kernel_size=3, stride=2 会将 7x7 变为 3x3
        pool_layer = nn.AvgPool2d(kernel_size=3, stride=2)

        # [bs, 768, 7, 7] -> [bs, 768, 3, 3]
        pooled_features = pool_layer(local_features_cf)

        # 5. 展平空间维度
        # [bs, 768, 3, 3] -> [bs, 768, 9] (9 = 3*3)
        pooled_features_flat = pooled_features.flatten(start_dim=2)

        # 6. 整理为 'channels-last' 格式 [bs, 9, 768]
        # [bs, 768, 9] -> [bs, 9, 768]
        local_features = pooled_features_flat.permute(0, 2, 1)
    ...
    def forward(self,
        ...
        wandb.init(
            ...
            name=f"dist_batchsize{self.batch_size * self.accumulate_step}_lr{self.lr}_shot{self.configs.num_shots}_epoch{self.epochs}",
            config=self.configs,
            # mode="offline",
        )
