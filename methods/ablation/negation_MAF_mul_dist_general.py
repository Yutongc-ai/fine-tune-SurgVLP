...
class NegationMulDistGen(nn.Module):
    ...
    def get_metrics(self, split):
        ...
        with torch.no_grad():
            ...
            for _ in range(len(feature_loader)):
                ...
                # logits = neg_logit / test_prompt_type_number * self.temperature
                logits = pos_logit * self.temperature
        ...
        final_logits = final_logits.mean().item()
        ...
        # print(metrics['mAP'])
        final_probs = final_probs.mean().item()
        metrics['mAP'] = final_probs
