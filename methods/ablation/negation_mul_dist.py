...
class NegationMulDistLast(nn.Module):
    ...
    def get_metrics(self, split):
        ...
        with torch.no_grad():
            ...
            for _ in range(len(feature_loader)):
                ...
                _, test_templates_feats, _ = self.model.extract_feat_text(ids = self.test_input_ids, attn_mask = self.test_attention_masks, token_type = self.test_token_type_ids)
                ada_test_feats_templates = self.text_adapter(test_templates_feats, self.alpha)
    ...
    def forward(self,
        ...
        _, train_templates_feats, _ = self.model.extract_feat_text(ids = self.train_input_ids, attn_mask = self.train_attention_masks, token_type = self.train_token_type_ids)
        ...
        for epoch in range(self.epochs):
            ...
            for i, (images, target, negated_target, _) in enumerate(tqdm(train_loader)):
                ...
                ada_negation_feats_templates = self.text_adapter(train_templates_feats, self.alpha)
