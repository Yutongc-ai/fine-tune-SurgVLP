...
class SingleHeadAttn(nn.Module):
    ...
    def attention(
        ...
        d_k = q_x.size(-1)
        attn_scores = torch.matmul(q_x, k_x.transpose(-2, -1))
        attn_scores = attn_scores / math.sqrt(d_k)
        
        if attn_mask is not None:
            attn_scores = attn_scores.masked_fill(attn_mask, 1e-9)
        attn_weights = F.softmax(attn_scores, dim=-1)

        output = torch.matmul(attn_weights, v_x)
    ...
    def forward(
        ...
        x = self.attention(q_x=q_x, k_x=k_x, v_x=v_x, attn_mask=attn_mask)[0]
        return x

class CrossAttnNoProj(nn.Module):
    def __init__(self, configs, model, preprocess, tokenizer):
        ...
        self.image_query_attn = SingleHeadAttn(self.feature_width).to(device)
        self.text_query_attn = SingleHeadAttn(self.feature_width).to(device)
    ...
    def forward(self,
        ...
        wandb.init(
            project=f"cross-attn-noproj-few-shot-surgvlp-{self.configs.model_config.type}",
