...
num_shots_options = [1, 2, 4, 8, 16, 32, 64]
epochs_options = [50]
learning_rates = [0.01]
annealling_sets = [True]
...
# 遍历每个组合并运行实验
for idx, (lr, annealling, attention_pooling, num_shot, epochs) in enumerate(all_combinations, 1):
    print(f"\n{'='*50}")
    print(f"Running experiment {idx}/{len(all_combinations)}")
    print(f"Config: attention_pooling={attention_pooling}, num_shots={num_shot}, epochs={epochs}, lr={lr}, annealling={annealling}")
    print(f"{'='*50}")
    
    # 构建命令行参数
    command = [
        "python", "few_shot.py",
        "--method", "negation_mul_dist_attn",
        "--random_select", "true",
        "--train_test", "true",
        "--attention_pooling", str(attention_pooling),
        "--num_shots", str(num_shot),
        "--model_type", "PeskaVLP",
        "--epochs", str(epochs),
        "--learning_rate", str(lr),
        "--annealling", str(annealling),
